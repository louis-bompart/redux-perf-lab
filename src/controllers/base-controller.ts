...
export function buildController(store: Store): Controller {
  let prevState: WeakRef<object>;
  const listeners: Map<Symbol, () => void> = new Map();
  const hasNoListeners = () => listeners.size === 0;

  const hasStateChanged = (currentState: object): boolean => {
    const hasChanged = prevState.deref() !== currentState;
    prevState = new WeakRef(currentState);
    return hasChanged;
  };
  ...
  return {
    subscribe(listener: () => void) {
      ...
      if (hasNoListeners()) {
        prevState = new WeakRef(this.state);
